    ...
    return(
            <li className="cart-item">
                <div className="cart-item-div">
                    ...
                    <p>{item.name} - {item.quantity} pz x {priceFormatter.format(item.price)}</p>
                    <p><strong>tot. {itemFormattedPrice}</strong></p>
                </div>
                <div className="cart-item-actions">
                    <Button onClick={() => handleUpdate(item, item.quantity+1)}>+</Button>
                    <span><input ref={quantityRef} value={quantity} onChange={handleChange} onBlur={handleBlur} onKeyPress={handleKeyPress}/></span>
                    <Button onClick={() => handleUpdate(item, item.quantity-1)}>-</Button>
